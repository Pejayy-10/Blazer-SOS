<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-4 border-l-4 {{ $platformStatus === 'open' ? 'border-green-500' : 'border-red-500' }}">
            <div class="flex items-center">
                <div class="flex-shrink-0 bg-indigo-500 dark:bg-indigo-600 rounded-md p-2">
                     {{-- Heroicon: status-online or status-offline --}}
                ...
                <div class="ml-4 flex-1">
                    ...
                    <p class="text-xl font-semibold text-gray-900 dark:text-gray-100">
                        {{ ucfirst($platformStatus) }}
                    </p>
                </div>
            </div>
            <div class="text-right mt-2">
                <a href="{{ route('admin.platforms.index') }}" wire:navigate class="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">Change Status</a>
            </div>
        </div>
         ...
         <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-4 border-l-4 border-blue-500">
             <div class="flex items-center">
                ...
                <div class="ml-4 flex-1">
                    ...
                    <p class="text-xl font-semibold text-gray-900 dark:text-gray-100">
                       {{ $totalProfilesSubmitted }}
                    </p>
                </div>
            </div>
              <div class="text-right mt-2">
                 {{-- Link might go to repository later --}}
                 <a href="{{ route('admin.subscriptions.index') }}" wire:navigate class="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">View All</a>
             </div>
        </div>

    </div> {{-- End Stats Grid --}}

    {{-- Optionally include Bulletin Board for admins too --}}
    <div class="mt-8">
        @livewire('bulletin')
    </div>
